...
// 2-1. GF(2^8) 상에서의 곱셈 역원 구하기-확장된 유클리드 알고리즘
// 다항식의 차수(최고차항 비트의 위치)를 구하는 함수
int Get_Poly_Degree(uint32_t poly) {
    int degree = -1;
    while (poly > 0) {
        degree++;
        poly >>= 1;
...
uint8_t GF_Inverse(uint8_t a) {
    if (a == 0) return 0; // 0의 역원은 0으로 정의됨

    uint32_t r0 = 0x11B; // AES 기약 다항식 m(x)
    uint32_t r1 = a;     // 역원을 구하고자 하는 입력 다항식 a(x)
    
    uint32_t t0 = 0;     // 초기 Bezout 계수 t0
    uint32_t t1 = 1;     // 초기 Bezout 계수 t1

    // 나머지가 1(최하위 차수 상수항만 남음)이 될 때까지 반복
    while (r1 != 1) {
        uint32_t q = 0;          // 몫 다항식
        uint32_t rem = r0;       // 나눗셈을 위한 임시 피제수(dividend) 변수
        int deg_r1 = Get_Poly_Degree(r1);

        // GF(2) 상에서 다항식 나눗셈 수행 (r0 / r1) -> 몫 q와 나머지 rem 산출
        while (1) {
            int deg_rem = Get_Poly_Degree(rem);
            if (deg_rem < deg_r1) break; // 피제수의 차수가 제수보다 낮아지면 나눗셈 종료

            int shift = deg_rem - deg_r1;
            q ^= (1 << shift);       // 몫(q)의 해당 차수 비트를 1로 설정 (XOR)
            rem ^= (r1 << shift);    // 피제수에서 제수를 시프트하여 감산 (XOR)
        }

        // 새로운 t 값 계산: t2 = t0 - q * t1 
        // GF(2^8) 상에서 뺄셈은 XOR이며, q와 t1의 곱셈은 기존의 Multiply 함수를 활용
        uint32_t t2 = t0 ^ Multiply((uint8_t)q, (uint8_t)t1);

        // 다음 단계를 위해 변수 스왑 (Euclidean Step)
        r0 = r1;
        r1 = rem;
        t0 = t1;
        t1 = t2;
...
void MixColumns(uint8_t state[4][4]) { 
    for (int c = 0; c < 4; ++c) {
        ...
        // Multiply 함수 대신 xtime을 사용하여 연산 속도와 수학적 직관성 극대화
...
void InvMixColumns(uint8_t state[4][4]) { // State의 각 열을 GF(2^8)에서 다항식 곱셈으로 변환
    for (int c = 0; c < 4; ++c) {
        ...
        uint8_t a2 = state[2][c];
        uint8_t a3 = state[3][c];
        // xtime을 사용하지 않고, Multiply 함수를 통해 다항식 곱셈을 수행하여 역 MixColumns를 구현
