...
// 10. 함수 이름 통일성 (접두어 적용) 및 2. const 적용
...
// 6. 다항식 곱셈 분리 (확장 유클리드 알고리즘에서의 수학적 엄밀성 확보)
...
// 1. S-box 생성 의존성 해결을 위한 명시적 초기화 함수
void AES_Init() {
    if (is_aes_initialized) return;
...
void AES_KeyExpansion(const uint8_t* key, uint8_t* RoundKey) {
    // 1. S-box 초기화 여부를 내부에서 스스로 확인 및 보장
...
// 7. 암호화 로직 특성상 Loop Unrolling을 통한 직접 치환 유지 (오버헤드 최소화)
